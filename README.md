...
- 各任教課程獨立的教學與評量進度
- 本機 AI 輔助介面與去識別化提醒（需自行設定 Gemini API 金鑰）；AI 行政助理把情境拆成任務／記事並抓日期時間，任務先進「草稿板」由老師拖曳決定主／子再一次建立；原文／附件中的網址自動寫入任務連結，點任務名稱可看完整說明與來源附件
- 📮 訊息整合站：LINE 群組訊息整段貼上，AI 拆條分類（轉達學生／家長／科任／自己待辦），勾選後整合成一則訊息
- 🎨 教材小工場（`Studio.html`）：學習單、作文引導單（含感官觀察四頁組）、作文批改（會考六級分、逐段批改與修改指引）、段考複習講義、閱讀分級改寫（可抓兒童適讀國際時事）、圖像簡報（每頁一張 Gemini 圖的 16:9 簡報，大綱→樣張→逐頁生成→單一 PDF）、心智圖、家長通知單／海報／LINE 圖文選單（Gemini 生圖）、Google 表單、座位表、敏感資料遮蔽、PDF 工具；選檔支援拍照／貼上，產出可下載 Word／Excel／PDF
...
一般老師不需要安裝 Python。請下載 [小綿助 Windows 專注版 v2.0.1](https://github.com/cona0815/teacher-dashboard/releases/download/xiaomianzhu-v2.0.1/xiaomianzhu-windows-portable.zip)，解壓縮後直接開啟 `XiaoMianZhu.exe`。

v2.0.1 SHA-256：`7aaba4af6c065b8eaa14c6b9623da34a57f1b8952ee7e88d5d37cdcfe2d7f123`
