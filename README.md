...
接著雙擊 `啟動小綿助秘書.bat`。桌面版預設使用本機模式，資料保存在使用者的 AppData；若已部署 GAS，可在小綿助首頁選填 `/exec` 網址與同步金鑰，測試成功後再啟用任務同步。連線失敗不會清除本機資料。

桌面小綿助的「本機記事」可按「匯出給網頁」產生 JSON，再到教師工作台的「小綿助記事」按「匯入桌面記事」。匯入只帶入記事文字、時間與附件檔名，不會把 Windows 本機附件路徑或原始檔案送進瀏覽器。設定 GAS 後可同步任務；記事與附件目前仍以本機保存及手動匯出為主，避免未確認內容自動上傳。

工作台「系統設定 → 小綿助」可調整啟用、自動執行、名稱、打氣話間隔、健康提醒、啟動簡報、桌面位置及安靜時段。按「下載桌面設定檔」後，將 `xiaomianzhu_settings.json` 放在 `小綿助.exe`（或 `desktop_pet_secretary.py`）旁邊並啟動一次；桌面版會匯入設定，並依選項新增或移除目前 Windows 使用者的登入啟動項目。
